...
class CReST():
    def __init__(self, num_cls, do_bg=False,  momentum=0.99):
    ...
    def _weight_wbg(self, num_each_class):
        ...
        mu_all = np.power((num_each_class+1e-8) / (np.amax(num_each_class)+1e-8), 1/3)
        index = [i for i in range(config.num_cls)]
        mu_all_dict = dict(zip(index, list(mu_all)))
        mu_all_dict = dict(sorted(mu_all_dict.items(),key=lambda s:s[1], reverse=True))
        for l in range(config.num_cls):
            idx = list(mu_all_dict.items())[l][0] # get the index of the majority class
            mu_l = list(mu_all_dict.items())
            weight[idx] = mu_l[config.num_cls-1-l][1] # eq (1)
        return weight

    def init_crest_weight(self, labeled_dataset):
...
if __name__ == '__main__':
    ...
    crest_A = CReST(config.num_cls, do_bg=True, momentum=0.99)
    crest_B = CReST(config.num_cls, do_bg=True, momentum=0.99)
    # crest_B = CReST()

    # make loss function
    weight_A = crest_A.init_crest_weight(labeled_loader.dataset)
    weight_B = crest_B.init_crest_weight(labeled_loader.dataset)
    print(weight_A)
    # print(torch.nn.functional.softmax(weight, dim=0))
    ...
    for epoch_num in range(args.max_epoch + 1):
        ...
        for batch_l, batch_u in tqdm(zip(labeled_loader, unlabeled_loader)):
            ...
            if args.mixed_precision:
                with autocast():
                    ...
                    weight_A = crest_A.get_weights(output_A_u)
                    weight_B = crest_B.get_weights(output_B_u)
                    # print(output_A)
                    # print(new_weight_A)
                    # print(softmax_weight)

                    # weight_A = EMA(new_weight_A, weight_A, momentum=0.99)
                    # weight_B = EMA(new_weight_B, weight_B, momentum=0.99)

                    # weight_A = weight_A / torch.max(weight_A)
                    # weight_B = weight_B / torch.max(weight_B)
                    ...
                    # softmax_weight = new_weight / torch.sum(new_weight)
                    # new_weight = torch.pow(torch.amax(softmax_weight) / softmax_weight, 1/3)
                        # new_weight = torch.cat([torch.tensor([1.0]).cuda(), new_weight])
                    # print("====",new_weight)
        ...
        # if epoch_num>0:
        logging.info(f"     Class Weights: {print_func(weight_A)}, lr: {get_lr(optimizer_A)}")

...
from data.transforms import RandomCrop, CenterCrop, RandomRotFlip, ToTensor, RandomFlip_LR, RandomFlip_UD
from data.data_loaders import Synapse_AMOS, Synapse_AMOS_light
...
def get_current_consistency_weight(epoch):
    if args.cps_rampup:
        ...
        return args.cps_w * sigmoid_rampup(epoch, args.max_epoch)
    else:
        return args.cps_w
...
class CLS:
    ...
    def init_weights(self, labeled_dataset):
        ...
        # self.weights = (self.momentum + (1- self.momentum) * weights) * self.num_cls
...
if __name__ == '__main__':
    ...
    # make data loader
    unlabeled_loader = make_loader(args.split_unlabeled, dst_cls=Synapse_AMOS_light, unlabeled=True)
    ...
    simis_A = CLS(config.num_cls, do_bg=True, momentum=0.99)
    simis_B = CLS(config.num_cls, do_bg=True, momentum=0.99)
    # make loss function
    weight_A = simis_A.init_weights(labeled_loader.dataset)
    weight_B = simis_B.init_weights(labeled_loader.dataset)
        ...
        for batch_l, batch_u in tqdm(zip(labeled_loader, unlabeled_loader)):
            ...
            if args.mixed_precision:
                with autocast():
                    ...
                    weight_A = simis_A.get_ema_weights(output_A_u)
                    weight_B = simis_B.get_ema_weights(output_B_u)
